...
export async function POST({ request }) {
  const { lng, lat, description, category, link, captchaToken } = await request.json();
  ...
  const BRITISH_ISLES_BOUNDS = { minLng: -10.6, maxLng: 1.8, minLat: 49.9, maxLat: 60.9 };
  if (lng < BRITISH_ISLES_BOUNDS.minLng || lng > BRITISH_ISLES_BOUNDS.maxLng || lat < BRITISH_ISLES_BOUNDS.minLat || lat > BRITISH_ISLES_BOUNDS.maxLat) {
    return json({ error: 'Location must be within the British Isles.' }, { status: 400 });
  }
